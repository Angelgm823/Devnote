...
class LabelRepository:
    ...
    def list_ids_for_owner_subset(self, owner_id: int, ids: list[int]) -> list[int]:
        ...
        return self.db.exec(select(Label.id).where(Label.owner_id == owner_id, Label.id.in_(set(ids)))).all()
    ...
    def list_note_ids_by_label_ids(self, label_ids: list[int]) -> list[int]:
        ...
        return self.db.exec(select(NoteLabelLink.note_id).where(NoteLabelLink.label_id.in_(label_ids))).all()
